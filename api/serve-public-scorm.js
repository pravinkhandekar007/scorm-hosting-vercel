...
export default async function handler(req, res) {
  ...
  // Set permissive CSP for SCORM content
  ...
  try {
    // Use SERVICE_ROLE_KEY to bypass RLS (same as private courses)
    // The token parameter will be validated on the frontend before making the request
    const { data, error } = await supabase.storage.from('scorm-packages').download(path);

    if (error || !data) {
      console.error('[serve-public-scorm] Download error:', { path, error });
      res.status(404).json({
    ...
    console.error('[serve-public-scorm] Exception:', { path, error: err.message });
    res.status(500).json({
